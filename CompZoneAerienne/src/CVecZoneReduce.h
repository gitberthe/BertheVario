...
public :
    ...
    void ReduceDistanceDroiteBigSmall(double DistanceMetresEcartDroite) ;
    void ReduceDistanceInPolygon(double DistanceMetresEcartDroite) ;

private :
    std::vector<st_coord_poly*> * m_pVecOrigine ;
    double GetDistanceDroite( st_coord_poly* pTADroite , st_coord_poly* pTBDroite , st_coord_poly* pTMilieu , st_coord_poly* pTProj = NULL ) ;
    double GetDistanceDroite(std::vector<st_coord_poly*> & Vec) ;
} ;
