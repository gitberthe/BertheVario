...
// init des variables
m_TerrainPosCur.m_AltiBaroRec = 0 ;
}
...
xTaskCreatePinnedToCore( TacheRelanceIgc, "RelanceIgc", RELANCE_IGC_STACK_SIZE, NULL  , RELANCE_IGC_PRIORITY ,
...
// mesure du voltage
m_MutexI2c.PrendreMutex() ;
for (int i = 0; i < 5; i++)
    Voltage += analogRead(VOLTAGE_DIVISOR_PIN);
m_MutexI2c.RelacherMutex() ;
Voltage /= 5; // 5 mesures
Voltage /= 1000 / 2 ; // mili volts sur pont diviseur

Voltage *= 0.98 ; // re-etalonnage intrinsec
...
{
float DiffAngle = GetDiffAngle( m_CapGpsDeg , m_QMC5883Mag.GetCapDegres() ) ;
if ( DiffAngle < -180. )
    DiffAngle = -180. ;
if ( DiffAngle > 180. )
    DiffAngle = 180. ;
