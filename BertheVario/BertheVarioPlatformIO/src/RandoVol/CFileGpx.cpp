...
if ( m_pVecTrack == NULL )
    m_pVecTrack = new CLGArray<StPoint> ;
else
    m_pVecTrack->SetSize(0) ;
m_pVecTrack->SetGrow( 100 ) ;
...
for ( int i = 0 ; i < m_pVecTrack->GetSize() ; i++ )
    {
...
m_Barycenter.m_Lat /= m_pVecTrack->GetSize() ;
m_Barycenter.m_Lon /= m_pVecTrack->GetSize() ;
...
else if ( AvecAlt && strstr( Ligne , "<ele>" ) != NULL && m_pVecTrack->GetSize() )
    {
    StPoint & Pts = (*m_pVecTrack)[m_pVecTrack->GetSize()-1] ;
    strtok( Ligne , "<>" ) ;
    strtok( NULL , "<>" ) ;
...
if ( AltitudeRest < 0 )
    AltitudeRest = 0 ;
for ( int i = iPtProche + 1 ; i < m_pVecTrack->GetSize() ; i++ )
    {
...
if ( AltitudeFait < 0 )
    AltitudeFait = 0 ;
for ( int i = 0 ; i < iPtProche && i < m_pVecTrack->GetSize() ; i++ )
    {
